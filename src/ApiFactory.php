...
class ApiFactory
{
    ...
    public function createService(Api $api, $serviceName)
    {
        ...
        foreach ($baseOperationData as $httpMethod) {
            ...
            if (isset($this->config['api-tools-content-validation'][$serviceClassName][$httpMethod])) {
                $validatorName = $this->config['api-tools-content-validation'][$serviceClassName][$httpMethod];
                if (isset($this->config['input_filter_specs'][$validatorName])) {
